...
def add_flight(flight_number, departure_airport_id, arrival_airport_id, departure_time, arrival_time, aircraft_id):
    ...
    cur = conn.cursor()
    try:
        sql_get_max_id = "SELECT MAX(FlightID) FROM airline.flights;"
        cur.execute(sql_get_max_id)
        max_flight_id = cur.fetchone()[0] 
        new_flight_id = max_flight_id + 1 if max_flight_id else 1  

        sql_insert = """
        INSERT INTO airline.flights (FlightID, FlightNumber, DepartureAirportID, ArrivalAirportID, DepartureTime, ArrivalTime, AircraftID)
        VALUES (%s, %s, %s, %s, %s, %s, %s);
        """
        cur.execute(sql_insert, (new_flight_id, flight_number, departure_airport_id, arrival_airport_id, departure_time, arrival_time, aircraft_id))
        conn.commit()  
        cur.close()
        conn.close()
        return True
...
def update_flight(flight_id, flight_number, departure_airport, arrival_airport, departure_time, arrival_time, aircraft_id):
    ...
    try:
        ...
        WHERE FlightID = %s;
        """
        cur.execute(sql, (flight_number, departure_airport, arrival_airport, departure_time, arrival_time, aircraft_id, flight_id))
        conn.commit()
        cur.close()
        conn.close()
        return True
    except Exception as e:
        print(f"Error updating flight with id {flight_id}: {e}")
        cur.close()
        conn.close()
        raise
...
def delete_flight(flight_id):
    ...
    try:
        sql = """
        DELETE FROM airline.flights WHERE FlightID = %s;
        """
        cur.execute(sql, (flight_id,))
        conn.commit()  
        cur.close()
        ...
        print(f"Error deleting flight with id {flight_id}: {e}")
        cur.close()
        conn.close()
        raise
...
def get_flight_summary_by_airport():
    ...
    try:
        sql = """
        SELECT DepartureAirportID, COUNT(*) AS FlightCount 
        FROM flights 
        GROUP BY DepartureAirportID;
        """
        cur.execute(sql) 
        ...
        print(f"Error fetching flight summary by airport: {e}")
        cur.close() 
        conn.close()
        raise  
