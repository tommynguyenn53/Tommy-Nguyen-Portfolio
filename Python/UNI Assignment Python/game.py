...
def cheese_shop_menu(hunter_name):
    ...
    while True:
        ...
        if option == "1":
            ...
            get_game_menu(hunter_name, gold, cheese, "Cardboard and Hook Trap")
        ...
        if travel and ord(travel[0]) == 27:
            ...
            if trap and ord(trap[0]) == 27:
                ...
                if hand == 'left':
                    ...
                    type_of_trap = "High Strain Steel Trap"
                elif hand == 'right':
                    print('Larry: Excellent choice.\nYour "Hot Tub Trap" is now set!')
                    print("Larry: You need cheese to attract a mouse.\nLarry places one cheddar on the trap!")
                    type_of_trap = "Hot Tub Trap"
...
def get_game_menu(hunter_name, gold: int, cheese: list, trap: str):
    while True:
        ...
        if menu_options == "2":
            ...
            added_gold = gold
            added_gold, points = hunt(gold, cheese, None, points)
            points += starting_points
            added_gold += gold
        if menu_options == "3":
