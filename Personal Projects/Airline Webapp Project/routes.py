...
def show_all_flights():
    ...
    try:
        ...
        flash(f"Error fetching flights: {str(e)}")
        return redirect(url_for('dashboard'))  


@app.route('/flight/search', methods=['POST', 'GET'])
def search_flight_by_id():
    ...
    if request.method == 'POST':
        ...
        if flight is None:
            flash(f"No flight found with ID: {flight_id}")
            return redirect(url_for('search_flight_by_id')) 

        page['title'] = f'Flight {flight_id}'
        return render_template('flight_detail.html', flight=flight, page=page, session=session)

    page['title'] = 'Search Flight by ID'
    return render_template('search_flight.html', page=page, session=session)
...
def add_flight():
    ...
    if not session.get('isadmin', False):
        flash("You do not have permission to add flights.")
        return redirect(url_for('show_all_flights'))
    ...
    if request.method == 'POST':
        ...
        if not flight_number or not departure_airport or not arrival_airport or not aircraft_id:
            flash("Flight number, departure airport, arrival airport, and aircraft ID are required.")
            return redirect(url_for('add_flight'))

        try:
            database.add_flight(flight_number, departure_airport, arrival_airport, departure_time, arrival_time, aircraft_id)
            flash("Flight added successfully!")
            return redirect(url_for('show_all_flights'))
        except Exception as e:
            flash(f"Error adding flight: {str(e)}")
            return redirect(url_for('add_flight'))
...
@app.route('/update-flight/<int:flight_id>', methods=['POST', 'GET'])
def update_flight(flight_id):
    ...
    if not session.get('isadmin', False):
        flash("You do not have permission to update flights.")
        return redirect(url_for('show_all_flights'))

    page['title'] = f'Update Flight {flight_id}'
    
    flight = database.get_flight_by_id(flight_id)  

    if not flight:
        flash(f"Flight with ID {flight_id} not found.")
        return redirect(url_for('show_all_flights'))
    ...
    if request.method == 'POST':
        ...
        try:
            database.update_flight(flight_id, flight_number, departure_airport, arrival_airport, departure_time, arrival_time, aircraft_id)
            flash(f"Flight {flight_id} updated successfully!")
            return redirect(url_for('show_all_flights'))
        except Exception as e:
            flash(f"Error updating flight: {str(e)}")
            return redirect(url_for('update_flight', flight_id=flight_id))
...
@app.route('/delete-flight/<int:flight_id>', methods=['GET'])
def delete_flight(flight_id):
    ...
    if not session.get('isadmin', False):
        flash("You do not have permission to delete flights.")
        return redirect(url_for('show_all_flights'))

    try:
        database.delete_flight(flight_id)
        flash(f"Flight with ID {flight_id} deleted successfully!")
    except Exception as e:
...
@app.route('/flight-summary', methods=['GET'])
def flight_summary():
   
    if 'logged_in' not in session or not session['logged_in']:
        return redirect(url_for('login'))

    page['title'] = 'Flight Summary by Airport'
    
    try:
        summary = database.get_flight_summary_by_airport() 
        return render_template('flight_summary.html',
                               summary=summary,
                               page=page,
                               session=session)
    except Exception as e:
        flash(f"Error fetching flight summary: {str(e)}")
        return redirect(url_for('dashboard'))
